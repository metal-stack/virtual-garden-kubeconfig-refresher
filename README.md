...
The Garden cluster can be hosted at metalstack.cloud, Google GKE, accessed via a ServiceAccount token, or reached via an in-cluster client. The connected account needs permissions to retrieve cluster credentials for the referenced cluster entity. If no credentials are provided, the program attempts to use an in-cluster client and checks there for the virtual-garden-access managed resource.
...
| GARDEN_CLUSTER_API_SERVER      | The Kubernetes API server URL of the cluster hosting Gardener           |
| GARDEN_CLUSTER_TOKEN           | A ServiceAccount bearer token for the cluster hosting Gardener          |
| GARDEN_CLUSTER_CA_DATA         | Base64-encoded CA certificate for the cluster hosting Gardener          |
